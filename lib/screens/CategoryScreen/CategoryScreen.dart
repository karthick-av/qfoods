...
class CategoryScreen extends StatefulWidget {
  final DishesHomeCategoriesModel dishesHomeCategoriesModel;
  const CategoryScreen({super.key, required this.dishesHomeCategoriesModel});
...
DishesHomeCategoriesModel? category;
...
bool header_loading = false;
 ...
 Provider.of<FilterSelectedProvider>(context, listen: false).ClearRestaurantFilterHandler();
      ...
      category = widget.dishesHomeCategoriesModel;
      setState(() { });
     if(widget.dishesHomeCategoriesModel?.withoutdetail ?? false){
       DishCategoryDetailHandler();
     }
    });
    scrollController.addListener(() { //listener 
     if(scrollController.position.maxScrollExtent == scrollController.offset){
      print("object");
       if(!ApiCallDone){
        BottomRestaurantsCategoryIdHandler();
       }
    }
...
Future<void> DishCategoryDetailHandler() async{
  DishesHomeCategoriesModel _category;
  header_loading= true;
  setState(() { });
try{
  
    var response = await http.get(Uri.parse("${ApiServices.get_by_category_Id}${widget?.dishesHomeCategoriesModel.categoryId}"));
     setState(() {
    loading = false;
  });
    if(response.statusCode == 200){
       ...
       _category = DishesHomeCategoriesModel.fromJson(response_body);
...
final filter = Provider.of<FilterSelectedProvider>(context, listen: false).RestaurantFilter;
  ...
  String Url = "${ApiServices.get_restaurants_category_id}${widget.dishesHomeCategoriesModel.categoryId}?page=${current_page}&per_page=${per_page}";
...
Future<void> FilterCategoryHandler()async {
...
Future<void> RestaurantsByCatIdHandler()async {
  List<RestaurantsCategoryModel> _restaurants= [];
  setState(() {
    loading = true;
  });
try{
    var response = await http.get(Uri.parse("${ApiServices.get_restaurants_category_id}${widget.dishesHomeCategoriesModel.categoryId}?page=${current_page}&per_page=${per_page}"));
    print(response.statusCode);
     setState(() {
    loading = false;
  });
...
Future<void> BottomRestaurantsCategoryIdHandler()async {
  print(current_page);
           ...
           title: Container(
             child: SingleChildScrollView(
              scrollDirection: Axis.horizontal,
               ...
               InkWell(
                onTap: (){
                  RestauarntFilterBottomSheet(context, FilterCategoryHandler);
                },
                 child: Container(
               ...
               delegate: SliverChildBuilderDelegate(
                 (BuildContext context, int index) {
                   return RestaurantsCategoryCard(
                    ...
                    restaurant: Restaurants[index], categoryId: category?.categoryId?.toString() ?? '',);
                 },
                 // 40 list items
       ...
       padding: const EdgeInsets.only(top:5.0, left: 3.0),
        ...
        child: Row(
          children: [
            ...
            maxLines: 1,
            overflow: TextOverflow.ellipsis
            ,style: TextStyle(color: AppColors.blackcolor, fontSize: ScreenUtil().setHeight(14.0), fontFamily: FONT_FAMILY, fontWeight: FontWeight.bold),)
          ],
        ),
      ),
    ),
